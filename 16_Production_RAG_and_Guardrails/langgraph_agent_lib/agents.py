...
def create_helpfulness_agent(
    ...
    """Create a LangGraph agent with post-response helpfulness evaluation.
    ...
    Args:
        ...
        tools: List of tools to bind to the model
        rag_chain: Optional RAG chain to include as a tool
        max_loops: Maximum number of messages before forced termination
    ...
    def helpfulness_node(state: AgentState) -> Dict[str, Any]:
        ...
        if len(state["messages"]) > max_loops:
